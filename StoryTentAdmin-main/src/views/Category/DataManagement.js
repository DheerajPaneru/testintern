...
changeStatus = async (id, status) => {
    try {
        ...
        const response = await Axios.patch(Constant.apiBasePath+'category/changeStatus/'+ id, {status: status}, { headers: { 'token': token }});
        let {data} = response;
...
deleteCategory = async event => {
    try {
        let token = getAccessToken();
        const response = await Axios.delete(Constant.apiBasePath+'category/' + event, { headers: { 'token': token }});
...
updateCategory = async event => {
    try {
        this.props.history.push('/update-category/'+ event); 
    }
    catch(err) {
        console.log(err);
    }
...
handleSubmit = async (event, sort) => {
    try {
        ...
        const response = await Axios.post(Constant.apiBasePath + 'category/get-all', {'key': selectValue, 'pageNo': event, 'searchKey': searchKey, 'sortByAttribute': sortByAttribute}, { headers: { 'token': token }});
...
render() {
    ...
    if(DataList.length > 0) {
        bodyData = DataList.map(el => {
            ...
            return <tr><td>{i}</td><td>{el.name}</td><td><img src={el.icon} alt="ind" style={{width:50,height:50}} key={el._id} resizemode="contain"/></td><td>{contestType}</td><td>{el.createdAt}</td><td><button className="btn btn-info" onClick = {()=>{this.changeStatus(el._id, el.status)}}>{status}</button></td><td><button className="btn btn-danger Banner-btn" onClick={() => {if(window.confirm('Delete the Category?'))
            {this.deleteCategory(el._id)}}}><i className="mdi mdi-delete" title='Delete Category'></i></button>
            <button className="btn btn-info Banner-btn edit-banner ml5" onClick={() => {this.updateCategory(el._id)}}>
            <i className="mdi mdi-square-edit-outline" title='Update Category'></i></button></td></tr>
        })
    }
                        ...
                        <div className="row">
                            <div className="col-6">
                                <div className="page-title-box">
                                    <h4 className="page-title-heading">Category Management</h4>
                                </div>
                            </div>
                            <div className="col-6">
                                <Link to="/add-category"><button type="button" className="btn btn-info mb-2 mr-2 float-right">
                                    Add New</button></Link>
                            </div>
                        </div>   
                        <div className="row">
