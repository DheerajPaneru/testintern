...
changeStatus = async (event) => {
    try {
        ...
        const response = await Axios.patch(Constant.apiBasePath+'story/changeStatus/'+ id, metaData, { headers: { 'token': token }});
        let {data} = response;
...
makeFeature = async (event, status) => {
    try {
        ...
        const response = await Axios.patch(Constant.apiBasePath + 'story/changeStatus/'+ event, metaData, { headers: { 'token': token }});
        let {data} = response;
        if(data.status === Constant.statusSuccess) {
            $('#gen-message').text(data.message);
            $('#gen-message').css('color', 'green');
...
delete = async event => {
    try {
        let token = getAccessToken();
        const response = await Axios.delete(Constant.apiBasePath+'story/' + event, { headers: { 'token': token }});
...
update = async event => {
    try {
        this.props.history.push('/update-story/'+ event); 
    }
    catch(err) {
        console.log(err);
    }
...
handleSubmit = async (event, sort) => {
    try {
        ...
        const response = await Axios.post(Constant.apiBasePath + 'story/get-all', {'key': selectValue, 'pageNo': event, 'searchKey': searchKey, 'sortByAttribute': sortByAttribute}, { headers: { 'token': token }});
...
render() {
    ...
    if(DataList.length > 0) {
        bodyData = DataList.map(el => {
            ...
            let buttonClass = 'btn btn-warning btn-sm tblbtns';
            let standardButtonClass = 'btn btn-warning btn-sm tblbtns';

            if(el.status === 1) {
                status = "Approved";
                buttonClass = 'btn btn-success btn-sm tblbtns';
            } 

            else if(el.status === 2) {
                status = 'Rejected';
                buttonClass = 'btn btn-danger btn-sm tblbtns';
            }

            if(el.standard === 1) {
                standard = "Freemium";
                standardButtonClass = 'btn btn-primary btn-sm tblbtns';
            }
            if(el.standard === 2) {
                standard = 'Premium';
                standardButtonClass = 'btn btn-success btn-sm tblbtns';
            }
            ...
            return <tr><td>{i}</td><td className='namecol'>{el.name}</td><td>{el.description}</td><td><img src={el.thumbnail} alt="ind" style={{width:50,height:50}} key={el._id} resizemode="contain"/></td><td>{el.createdAt}</td><td><button className={ buttonClass }  onClick = {()=>{this.openModal(el._id, 1, el.status, el.standard)}}>{status}</button></td>
            <td><button className={ standardButtonClass } onClick = {()=>{this.openModal(el._id, 2, el.status, el.standard)}}>{standard}</button></td>
            <td>
                <button className="btn btn-info tblbtns" onClick = {()=>{this.makeFeature(el._id, el.feature, 2)}}>{feature}</button>                 
            </td>
            <td><Link to={filmURL}><button className='btn btn-primary tblbtns'>Film</button></Link></td>
            <td><button className="btn btn-danger Banner-btn updatebtns" onClick={() => {if(window.confirm('Delete the story?'))
            {this.delete(el._id)}}}><i className="mdi mdi-delete" title='Delete Story'></i></button>
            <button className="btn Banner-btn edit-banner ml5 btn-primary updatebtns" onClick={() => {this.update(el._id)}}>
        ...
        $(".export-content-btn").on("click", async () => {
            try {
                let token = getAccessToken();
                var {data} = await Axios.post(Constant.apiBasePath + 'story/get-all', { 'exportData': 1 }, { headers: { token: token }});
                if(data.status === Constant.statusSuccess) {
                    var csvString = data.data;
                    var universalBOM = "\uFEFF";
                    var a = window.document.createElement('a');
                    a.setAttribute('href', 'data:text/csv; charset=utf-8,' + encodeURIComponent(universalBOM+csvString));
                    a.setAttribute('download', 'stories.csv');
                        ...
                        <div className="row">
                            <div className="col-6">
                                <div className="page-title-box">
                                    <h4 className="page-title-heading">Story Management</h4>
                                </div>
                            </div>
                            <div className="col-6">
                                <Link to="/create-story"><button type="button" className="btn btn-info mb-2 mr-2 float-right">
                                    Add New Story</button></Link>
                            </div>
                        </div>   
                        <div className="row">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title">Story Model</h4>
