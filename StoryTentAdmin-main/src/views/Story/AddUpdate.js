...
/*..................................... default state object ............................................*/
    ...
    uploadThumbnail = async (e) => {
        ...
        if(file) {
            ...
            formData.append('story', file, file.name);
            let { data } = await Axios.post(Constant.apiBasePath + 'story/uploadFileOnS3', formData, { headers: { 'token': token }});
    ...
    uploadDocument = async (e) => {
        ...
        if(file) {
            let token = localStorage.getItem('loginDetails');
            let formData = new FormData();
            formData.append('storyDocument', file, file.name);
            let { data } = await Axios.post(Constant.apiBasePath + 'story/uploadFileInArray', formData, { headers: { 'token': token }});
            if(data.status === Constant.statusSuccess) {
    ...
    handleSubmit = async (event) => {
        try {
            ...
            if(id === undefined) {
                ...
                const response = await Axios.post(Constant.apiBasePath+'story/create', metaData, { headers: { 'token': token }});
                let {data} = response;
                if(data.status === Constant.statusSuccess) {
                    $('#gen-message').text(data.message);
                    $('#gen-message').css('color', 'green');
                    setTimeout(function() { window.location.href = '/story-management'; }, 2000 );
                }
                else {
                    $('#gen-message').text(data.message);
    ...
    async getDataById(id) {
        ...
        let req = {
            method: 'GET',
            url: Constant.apiBasePath + 'story/getDataById/' + id,
            headers:
            {
        ...
        Axios(req).then(response => {
            ...
            if (data.status === Constant.statusSuccess) {
                ...
                this.setState({ GetData: getData, selectedOption: selectedOption, ERROR: false });
            }
            else {
                $('#gen-message').text(data.message);
                $('#gen-message').css('color', 'red');
                                ...
                                <div className="page-title-box">
                                    <div className="page-title-right">
                                        <ol className="breadcrumb m-0">
                                            <li className="breadcrumb-item"><Link to="/story-management">Story Management</Link></li>
