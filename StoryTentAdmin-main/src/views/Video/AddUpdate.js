...
/*.................. Integrate API for add, update data...............................................*/
        ...
        try{
            ...
            if (id === undefined) {
                ...
                if(this.state.video === null) {
                    $("#error-video-msg").text('video is required');
                    $("#error-video-msg").css("color", "red");
                    return false;
                }
                else{
                    $("#error-video-msg").text('');
                }
    
                let thumbName = this.state.thumbnail.name;
                let thumb_extension = thumbName.split('.').pop();

                let videoName = this.state.video.name;
                let video_extension = videoName.split('.').pop();
                ...
                if(video_extension !== 'mp4') {
                    $("#error-video-msg").text('Only mp4 file type is allow !');
                    $("#error-video-msg").css("color", "red");
                    return false;
                }
                ...
                Axios.post(Constant.apiBasePath + 'video/create', form_data, { headers: { 'token': token }}).then(response => {
                    ...
                    if(data.status === Constant.statusSuccess) {
                        ...
                        setTimeout(function() { window.location.href = '/video-management'; }, 3000 );
                    }
                    else{
                        $('#gen-message').text(data.message);
                ...
                (this.state.video) ? form_data.append('video', this.state.video, this.state.video.name) : form_data.append('videoURL', GetData.videoURL);
                ...
                Axios.put(Constant.apiBasePath + 'video/update/' + id, form_data, { headers: { 'token': token }}).then(response => {
                    let data = response.data;
                    this.setState({ 'isLoading': false, disabled: false });
    ...
    async getGetData(id) {
        ...
        let req = {
            method: 'GET',
            url: Constant.apiBasePath + 'video/getDataById/' + id,
            headers:
            {
                                ...
                                <div className="page-title-box">
                                    <div className="page-title-right">
                                        <ol className="breadcrumb m-0">
                                            <li className="breadcrumb-item"><Link to="/video-management">Video Management</Link></li>
...

                                            {
                                                ...
                                                ?
                                                    <>
                                                        <div className="col-md-6">
                                                            <div className="form-group mb-3">
                                                                <label htmlFor="state" className="state">Upload video *</label>
                                                                <input type="file" id="video" className="form-control" accept="video/mp4,video/x-m4v,video/*" onChange={this.handleFile}/>
                                                                <p id="error-video-msg"/>
