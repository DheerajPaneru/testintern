...
class AudioManagement extends Component {
    ...
    updateAudio = (event) => {
        this.props.history.push('/update-audio/' + event);
    }
...
changeStatus = async (event, status, type) => {
    try {
        ...
        const response = await Axios.patch(Constant.apiBasePath + 'audio/changeStatus/'+ event, metaData, { headers: { 'token': token }});
        let {data} = response;
...
delete = async event => {
    try {
        let token = getAccessToken();
        const response = await Axios.delete(Constant.apiBasePath + 'audio/delete/' + event, { headers: { 'token': token }});
...
handleSubmit = async (event, sort) => {
    try {
        ...
        const response = await Axios.post(Constant.apiBasePath + 'audio/get-all', { 'key': selectValue, 'pageNo': event }, { headers: { 'token': token }});
        const {data, length, pageNo} = response.data;
        this.setState({AudioList: data, length: length, pageNo: pageNo, defaultPageNo: event, selectValue:'', 'searchKey': '', ERROR: false});
    }
    catch(err) {
        console.log(err);
    }
...
render() {
    const {AudioList, length, pageNo} = this.state;
    let i = (pageNo - 1) * Constant.perPage;

    let bodyData = '';
    if(AudioList.length > 0) {
        bodyData = AudioList.map(el => {
            i++;
            let status = "Active";
            if(el.status === 0) {
                status = "Inactive";
            ...
            <td><button className="btn btn-danger Banner-btn updatebtns" onClick={() => {if(window.confirm('Delete the audio?'))
            {this.delete(el._id)}}}><i className="mdi mdi-delete" title='Delete audio'></i></button>
            <button className="btn btn-info Banner-btn edit-banner ml5 updatebtns" onClick={() => {this.updateAudio(el._id)}}>
            <i className="mdi mdi-square-edit-outline" title='Update audio'></i></button>
            </td></tr>
        })
    }
                        ...
                        <div className="row">
                            <div className="col-8">
                                <div className="page-title-box">
                                    <h4 className="page-title-heading">Audio Management</h4>
                                </div>
                            </div>
                            <div className="col-4 text-right">
                                <Link to="/add-audio"><button type="button" className="btn btn-info">
                                    Add New Data</button>
                                </Link>
                            </div>
                        </div>  
