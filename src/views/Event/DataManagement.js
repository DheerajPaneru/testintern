...
changeStatus = async (id, status) => {
    try {
        ...
        const response = await Axios.patch(Constant.apiBasePath+'event/changeStatus/'+ id, { status: status }, { headers: { 'token': token }});
        let {data} = response;
...
deleteEvent = async (event, thumbnail) => {
    try {
        let token = getAccessToken();
        const response = await Axios.delete(Constant.apiBasePath+'event/delete/' + event + '?thumbnail=' + thumbnail, { headers: { 'token': token }});
...
handleSubmit = async (event, sort) => {
    try {
        ...
        const response = await Axios.post(Constant.apiBasePath + 'event/get-all', { 'key': selectValue, 'pageNo': event }, { headers: { 'token': token }});
...
render() {
    ...
    if(DataList.length > 0) {
        bodyData = DataList.map(el => {
            ...
            return <tr><td>{i}</td><td>{el.name}</td><td>{el.venue}</td><td>{el.date}</td><td>{el.time}</td><td>{el.createdAt}</td><td><button className="btn btn-info" onClick = {()=>{this.changeStatus(el._id, el.status)}}>{status}</button></td><td><button className="btn btn-danger Banner-btn" onClick={() => {if(window.confirm('Delete the Event?'))
            {this.deleteEvent(el._id, el.thumbnail)}}}><i className="mdi mdi-delete" title='Delete Thumbnail'></i></button>
            </td></tr>
        })
    }
                        ...
                        <div className="row">
                            <div className="col-6">
                                <div className="page-title-box">
                                    <h4 className="page-title-heading">Event Management</h4>
                                </div>
                            </div>
                            <div className="col-6">
                                <Link to="/create-event"><button type="button" className="btn btn-info mb-2 mr-2 float-right">
                                    Add New</button></Link>
                            </div>
                        </div>   
                        <div className="row">
